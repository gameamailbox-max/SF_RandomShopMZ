 ...
 * @text SOLD-OUT文字色
 * @desc 購入済み商品のSOLD-OUT表示の文字色です。\C[n]の形式で指定してください。
 * @type string
 * @default \C[8]
 *
 * @param SoldOutText
 * @text 売り切れ表示文字
 * @desc 購入済み商品の価格欄に表示する文字です。
 * @type string
 * @default SOLD-OUT
 *
 ...
 *
 * 商品は1種類につき1個のみ購入できます。
 * 購入済みの商品は一覧に残り、商品名はグレーアウトし、
 * 価格欄がSOLD-OUT表示に切り替わります。
 *
...
(() => {
    ...
    const pluginName = "SF_RandomShopMZ_Ver1.1";
    const legacyPluginName = "SF_RandomShopMZ";
    ...
    const soldOutText =
        String(
            parameters["SoldOutText"] || "SOLD-OUT"
        );
    ...
    PluginManager.registerCommand(
        pluginName,
        "SetupRandomShop",
        setupRandomShopCommand
    );

    // 旧版で設定済みのイベントコマンドとの互換性を維持
    PluginManager.registerCommand(
        legacyPluginName,
        "SetupRandomShop",
        setupRandomShopCommand
